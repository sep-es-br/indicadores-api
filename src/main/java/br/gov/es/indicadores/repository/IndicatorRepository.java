...
public interface IndicatorRepository extends Neo4jRepository<Indicator, String> {
   ...
   @Query(value = "MATCH (n:Indicator) WHERE apoc.text.clean(n.name) CONTAINS apoc.text.clean($name) " +
         "RETURN n.uuId AS uuId, toUpper(n.name) AS name " +
         "ORDER BY apoc.text.clean(n.name) ASC " +
         "SKIP $skip LIMIT $limit", countQuery = "MATCH (n:Indicator) WHERE apoc.text.clean(n.name) CONTAINS apoc.text.clean($name) RETURN count(n)")
    ...
    * " WITH i, rm, coalesce(apoc.coll.sort(collect(DISTINCT ods.order)), []) AS ods, "
    * +
    ...
    * +
    * " collect(DISTINCT CASE WHEN resultTime.year IS NOT NULL AND (r2.value IS NOT NULL OR r2.showValue IS NOT NULL) "
    * +
    ...
    * )
    * List<IndicatorDto> indicatorByChallenge(@Param("challengeUuId") String
    * challengeUuId);
    */
